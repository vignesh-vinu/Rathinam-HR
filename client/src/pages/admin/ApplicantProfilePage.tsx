  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 animate-fadeIn pb-24">
      ...
      <div className="no-print flex flex-col md:flex-row md:items-center justify-between gap-4">
        <button
          onClick={() => onNavigate('admin-dashboard')}
          className="flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-white hover:bg-sky-50 text-slate-700 text-xs font-bold border border-sky-200 shadow-sm transition-colors"
        >
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-extrabold text-xs shadow-md shadow-sky-500/20 transition-all hover:scale-105"
          >
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${
              activeTab === 'resume' ? 'bg-sky-600 text-white font-extrabold shadow-md shadow-sky-500/20' : 'bg-white text-sky-800 hover:bg-sky-50 border border-sky-200'
            }`}
          >
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 rounded-xl bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white font-extrabold text-xs shadow-lg shadow-sky-500/25 transition-all hover:scale-105"
          >
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 rounded-xl bg-sky-50 hover:bg-sky-100 text-sky-800 font-bold text-xs border border-sky-200 transition-colors"
          >
      ...
      {/* Success Toast */}
      {successToast && (
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-800 text-xs font-bold flex items-center justify-between animate-fadeIn shadow-sm">
      ...
      {/* HEADER CANDIDATE BANNER - White & Sky Blue Theme */}
      <div className="p-6 sm:p-8 rounded-3xl border border-sky-300 bg-gradient-to-r from-sky-600 via-sky-500 to-blue-600 text-white shadow-xl shadow-sky-500/15 no-print">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center space-x-5">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="px-2.5 py-0.5 text-xs font-extrabold rounded-md bg-white/20 text-white border border-white/30 backdrop-blur-sm">
                  {app.organizationId}
                </span>
                <span className="font-mono text-xs text-sky-100 font-bold">
                  {app.applicationId}
                </span>
              </div>
          ...
          <div className="text-center md:text-right space-y-2">
            <span className="px-4 py-1.5 rounded-full text-xs font-extrabold uppercase tracking-wider inline-block bg-white text-sky-900 shadow-sm">
              Status: {app.status}
            </span>
            <p className="text-xs font-bold text-white">
              Submitted: <span className="text-sky-100 font-mono">{app.submissionDate || new Date(app.submittedAt).toLocaleDateString()}</span> at <span className="text-sky-100 font-mono">{app.submissionTime || new Date(app.submittedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            </p>
          </div>
      ...
      <div className="no-print flex items-center space-x-2 border-b border-sky-200 pb-2 overflow-x-auto">
        {[
          { id: 'overview', label: 'Full Overview', icon: User },
          { id: 'resume', label: 'Executive Data Sheet PDF', icon: FileText },
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                isActive ? 'bg-sky-600 text-white font-extrabold shadow-md shadow-sky-500/20' : 'bg-white text-slate-700 hover:bg-sky-50 hover:text-sky-700 border border-sky-200/80 font-bold'
              }`}
            >
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="glass-panel p-6 rounded-2xl border border-sky-200 space-y-3 bg-white shadow-sm">
              <h3 className="text-sm font-bold text-sky-800 uppercase tracking-wider flex items-center space-x-2">
                <User className="w-4 h-4 text-sky-600" />
                <span>Personal & Bio Details</span>
              </h3>
              <div className="space-y-2 text-xs text-slate-700 font-medium">
                <p><strong className="text-slate-900 font-semibold">Full Name:</strong> {p.fullName}</p>
                <p><strong className="text-slate-900 font-semibold">DOB / Age:</strong> {p.dob} ({p.age} Years)</p>
                <p><strong className="text-slate-900 font-semibold">Gender / Marital Status:</strong> {p.gender} / {p.maritalStatus}</p>
                <p><strong className="text-slate-900 font-semibold">Nationality / Category:</strong> {p.nationality} ({p.category})</p>
              </div>
            </div>
            ...
            <div className="glass-panel p-6 rounded-2xl border border-sky-200 space-y-3 bg-white shadow-sm">
              ...
              <div className="space-y-2 text-xs text-slate-700 font-medium">
                <p><strong className="text-slate-900 font-semibold">Email ID:</strong> {c.email}</p>
                <p><strong className="text-slate-900 font-semibold">Mobile / Alt Phone:</strong> {c.mobile} / {c.phone || 'N/A'}</p>
                <p><strong className="text-slate-900 font-semibold">Address:</strong> {c.address}, {c.city}, {c.state} - {c.pincode}</p>
              </div>
            </div>
          ...
          {/* Education Table */}
          <div className="glass-panel p-6 rounded-2xl border border-sky-200 space-y-4 bg-white shadow-sm">
            <h3 className="text-sm font-bold text-sky-800 uppercase tracking-wider flex items-center space-x-2">
                ...
                <tbody className="divide-y divide-sky-100">
                  {(app.educationDetails || []).map((edu, idx) => (
                    <tr key={idx} className="hover:bg-sky-50/50">
                      <td className="py-2.5 px-3 font-bold text-slate-900">{edu.degree}</td>
                      <td className="py-2.5 px-3 text-slate-700">{edu.institution} ({edu.boardUniversity})</td>
                      <td className="py-2.5 px-3 text-slate-600">{edu.division}</td>
                      <td className="py-2.5 px-3 text-slate-700">{edu.majorSubjects}</td>
                      <td className="py-2.5 px-3 font-mono text-sky-700 font-bold">{edu.yearOfPassing}</td>
                      <td className="py-2.5 px-3 font-bold text-emerald-700">{edu.percentage}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-sky-100">
                  {(app.experienceDetails || []).map((exp, idx) => (
                    <tr key={idx} className="hover:bg-sky-50/50">
                      <td className="py-2.5 px-3 font-bold text-slate-900">{exp.organization}</td>
                      <td className="py-2.5 px-3 text-sky-700 font-bold">{exp.designation}</td>
                      <td className="py-2.5 px-3 text-slate-600">{exp.periodFrom} to {exp.periodTo}</td>
                      <td className="py-2.5 px-3 font-mono text-slate-800 font-bold">₹{exp.grossAnnualSalary}</td>
                      <td className="py-2.5 px-3 text-slate-600">{exp.reasonForLeaving}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === 'documents' && (
        <div className="space-y-6 animate-fadeIn">
          <div className="glass-panel p-6 rounded-3xl border border-sky-200 space-y-4 bg-white shadow-sm">
            <h3 className="text-sm font-bold text-sky-800 uppercase tracking-wider flex items-center space-x-2">
            ...
            {(app.documents || []).length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(app.documents || []).map((doc) => (
                  <div key={doc.id} className="p-4 rounded-2xl bg-sky-50/60 border border-sky-200 flex items-center justify-between hover:border-sky-400 transition-colors shadow-sm">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 rounded-xl bg-white border border-sky-200 flex items-center justify-center shadow-inner">
                    ...
                    <div className="flex items-center space-x-2">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="px-3 py-1.5 rounded-lg bg-sky-600 hover:bg-sky-500 text-white font-bold text-xs flex items-center space-x-1 shadow-sm"
                      >
                      ...
                      <a
                        href={doc.url}
                        download={doc.name}
                        className="p-1.5 rounded-lg bg-white border border-sky-200 hover:bg-sky-50 text-sky-700"
                        title="Download"
                      >
        ...
        <div className="space-y-6 animate-fadeIn">
          <div className="glass-panel p-6 rounded-3xl border border-sky-200 space-y-4 bg-white shadow-sm">
            ...
            {history.length === 0 ? (
              ...
              <div className="space-y-4 relative pl-4 border-l-2 border-sky-200">
                {history.map((item, idx) => (
                  <div key={item.id || idx} className="relative group">
                    <div className="absolute -left-[23px] top-1.5 w-3 h-3 rounded-full bg-sky-600 ring-4 ring-white" />
                    <div className="p-4 rounded-2xl bg-sky-50/60 border border-sky-200 space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-sky-800">{item.toStatus}</span>
                        <span className="text-[10px] text-slate-500">{new Date(item.timestamp).toLocaleString()}</span>
                      </div>
                      <p className="text-xs text-slate-700 mt-1">{item.remarks}</p>
                      <p className="text-[10px] text-slate-500 pt-1">Changed by: <strong className="text-slate-800">{item.updatedBy}</strong></p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTab === 'notes' && (
        <div className="space-y-6 animate-fadeIn">
          <div className="glass-panel p-6 rounded-2xl border border-sky-200 space-y-4 bg-white shadow-sm">
            ...
            <form onSubmit={handleAddNote} className="space-y-3">
              <textarea
                ...
                className="w-full px-4 py-2.5 rounded-xl glass-input text-xs text-slate-900 border border-sky-200"
              />
              <button
                type="submit"
                disabled={addingNote}
                className="px-5 py-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-bold text-xs shadow-md shadow-sky-500/20"
              >
          ...
          <div className="space-y-3">
            ...
            {notes.map(note => (
              <div key={note.id} className="p-4 rounded-xl bg-sky-50/60 border border-sky-200 space-y-1 text-xs shadow-sm">
                ...
                <p className="text-slate-800 mt-1">{note.content}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-fadeIn overflow-y-auto">
          <form onSubmit={handleSaveHREdits} className="glass-panel p-6 sm:p-8 rounded-3xl max-w-2xl w-full border border-sky-300 bg-white space-y-5 my-8 shadow-2xl">
            <div className="flex items-center justify-between border-b border-sky-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Correct any errors or mistakes made during candidate submission.</p>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl glass-input text-xs text-slate-900 border border-sky-200 uppercase"
                />
              </div>
            ...
            <div className="flex items-center justify-end space-x-3 pt-4 border-t border-sky-100">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-extrabold text-xs shadow-md shadow-sky-500/20 flex items-center space-x-2"
              >
